...
#模拟数据字典
dict_data={ 'data1.txt':['id','name1','at1','at2'],
            'data2.txt':['id','bt1','bt2','name2','bt3','bt4','bt5']}
#where子句中目前只处理>,<,=,<>
char_in_where={'<>':0,'>':1,'<':2,'=':3}
#属性名转换成数字
attr_to_int={'data1.txt':{'id':0,'name1':1,'at1':2,'at2':3},
            'data2.txt':{'id':0,'bt1':1,'bt2':2,'name2':3,'bt3':4,'bt4':5,'bt5':6}}
table_to_int={'data1.txt':0,'data2.txt':2}#按照表的大小排列
...
print("属性为：",select_name,distinct)
...
while line1:
    ...
    cnt2+=1
    line1=file1.readline()
print("两个表的大小分别是：",cnt1, "行、",cnt2,"行")
timeStart=time.time()#开始计时
#先对两个表按照连接属性排序,默认升序排列
table1=table1.sort_values(by='name1')
...
while i_1 < cnt1:
    if i_2 == cnt2:
        break
    while i_2 < cnt2:
        if table1['name1'][i_1] < table2['name2'][i_2]:#若左边表的连接值比右边表小，则跳过左边的表
            i_1+=1
        elif table1['name1'][i_1] > table2['name2'][i_2]:#若右边表的连接值比左边小，则跳过右边的表
            i_2+=1
...
if distinct==True:
    ...
    result.index=[k for k in range(0,len(result))]

result.to_csv("result.csv",mode='a',encoding='UTF-8')
timeEnd=time.time()#结束计时
runTime=float('%.2f'%(timeEnd-timeStart))
print("归并连接完成！用时",runTime,"s")

#测试样例
"""
select name1, bt4, bt5
from data2,data1
where data1.name1 = data2.name2;
"""